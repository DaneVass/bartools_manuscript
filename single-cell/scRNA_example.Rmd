...
MLL-AF9 cells were barcoded with the SPLINTR mCHERRY barcode library (ref). After expansion for 1 week the cell pool was divided in half and each was separately labelled with a different Mouse TotalSeq-A hashtag. Following this, pools were combined and 10X Genomics single-cell RNA-seq 3' v3 sample was performed.  

A representative sample of the same cells were also harvested for population DNA barcoding (barcode-seq) at this timepoint with the idea that we can compare the total barcode frequency in the population to those recovered in the single cell data to get an idea of the barcode recovery before we do our main drug resistance experiment.
...
```{r}
bc.annot <- read.delim("../../datasets/barcoded_single_cell/BARtab_out/counts/unmapped_cell-barcode-anno.tsv", sep = ' ')
...
percent.mito <- Matrix::colSums(sc@assays$RNA[mito.genes, ])/Matrix::colSums(sc@assays$RNA)*100
...
#VlnPlot(object = sc, features = c("nFeature_RNA", "nCount_RNA", "percent.mito"), ncol = 3, pt.size = 0, log = F, group.by = "detected", cols = c("firebrick", "red2"), split.by = "HTO_maxID")
#ggsave("./plots/QC_metrics_pre_filter_detected_HTO.pdf")
...
sc <- HTODemux(sc, assay = "HTO", positive.quantile = 0.99, seed = 10101, verbose = T)
...
```{r}
bartools::plotCellsPerGroup(sc.obj = sc, group = "barcode", order = F, threshold = 100)
```
...
ggtheme_dv <- function(base_size = 11, base_family = "", base_line_size = base_size/22, 
...
sc@meta.data %>% dplyr::select(doubletBarcode, HTO_classification.global, MA9_1_HTO_norm, MA9_2_HTO_norm, nCount_RNA) %>% filter(doubletBarcode == "singlet") %>% ggplot(aes(HTO_classification.global, nCount_RNA, fill = HTO_classification.global)) +
  geom_violin() + geom_boxplot(width = 0.1, fill = "white", outlier.shape = NA) + ggtitle("Barcode singlets") + ggtheme_dv()
...
sc@meta.data %>% dplyr::select(doubletBarcode, HTO_classification.global, MA9_1_HTO_norm, MA9_2_HTO_norm, nCount_RNA) %>% filter(doubletBarcode == "doublet") %>% ggplot(aes(HTO_classification.global, nCount_RNA, fill = HTO_classification.global)) +
  geom_violin() + geom_boxplot(width = 0.1, fill = "white", outlier.shape = NA) + ggtitle("Barcode doublets") + ggtheme_dv() + scale_fill_manual(values =  ggpubr::get_palette("aaas", k = 10))
...
SPLINTR.NA <- rownames(sc@meta.data[which(is.na(sc$barcode)),])
SPLINTR.detected <- rownames(sc@meta.data[which(!is.na(sc$barcode)),])

SPLINTR.unknown <- rownames(sc@meta.data[which(sc$doubletBarcode == "unknown"),])
SPLINTR.singlet <- rownames(sc@meta.data[which(sc$doubletBarcode == "singlet"),])
SPLINTR.doublet <- rownames(sc@meta.data[which(sc$doubletBarcode == "doublet"),])

HTO.unknown <- rownames(sc@meta.data[which(sc$HTO_classification.global == "Negative"),])
HTO.singlet <- rownames(sc@meta.data[which(sc$HTO_classification.global == "Singlet"),])
HTO.doublet <- rownames(sc@meta.data[which(sc$HTO_classification.global == "Doublet"),])

res <- venndetail(list("SPLINTR.doublet" = SPLINTR.doublet,
                       "SPLINTR.unknown" = SPLINTR.unknown,
                       "HTO.doublet" = HTO.doublet,
                       "HTO.unknown" = HTO.unknown))
vennpie(res)
plot(res)
...
RidgePlot(sc, features = c("Pcna", "Top2a", "Mcm6", "Mki67"), ncol = 2)
ggsave("./plots/ridgeplot_cell_cycle_labelled.pdf")
```
...
## filter scRNAseq object
```{r}
Idents(sc) <- "HTO_classification.global"
# Remove negative HTO cells from the object
sc.filter <- subset(sc, idents = "Negative", invert = TRUE)
...
## filter single cell data on QC metrics
```{r}
sc.filter <- subset(sc.filter, subset = nFeature_RNA > 500 & nFeature_RNA < 6000 & percent.mito < 10)
```

## plot basic scRNA-seq metrics - post-filter
```{r}
...
VlnPlot(object = sc.filter, features = c("nFeature_RNA", "nCount_RNA", "percent.mito"), ncol = 3, pt.size = 0, log = F, group.by = "detected", cols = c("firebrick", "red2"), split.by = "HTO_maxID")
ggsave("./plots/QC_metrics_post_filter_detected_HTO.pdf")
...
dat <- sc.filter@meta.data %>% dplyr::select(nCount_RNA, nFeature_RNA, percent.mito, detected, doubletBarcode, bc.umi.count)
dat$detected <- as.factor(dat$detected)
dat$doubletBarcode <- as.factor(dat$doubletBarcode)
...
ggplot(dat, aes(x = nCount_RNA, y = nFeature_RNA, color = bc.umi.count)) + 
  geom_point(size = 0.5) + 
  scale_color_continuous(magma(30)) +
  theme_bw()
...
sc.filter <- ScaleData(sc.filter, features = rownames(sc.filter), vars.to.regress = c("S.Score", "G2M.Score"))
...
```{r}
sc.filter <- readRDS(file = "../scRNA-seq_example/results/sc_filtered_normalised_seurat.rds")
sc.filter.sce <- readRDS(file = "../scRNA-seq_example/results/sc_filtered_normalised_sce.rds")
